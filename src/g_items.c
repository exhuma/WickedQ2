...
void SpawnItem (edict_t *ent, gitem_t *item)
{
	...
	if (deathmatch->value)
	{
		if ( strcmp(ent->classname, "weapon_shotgun") == 0 ) {
			...
			G_FreeEdict (ent);
			return;
		} else if ( strcmp(ent->classname, "ammo_shells") == 0 ) {
			G_FreeEdict (ent);
			return;
			...
			G_FreeEdict (ent);
			return;
		} else if ( strcmp(ent->classname, "item_bandolier") == 0 ) {
			G_FreeEdict (ent);
			return;
		} else if ( strcmp(ent->classname, "item_pack") == 0 ) {
			G_FreeEdict (ent);
			return;
		}
...
int AmmoIndex(gitem_t *weapon) {
  ...
  if (weapon == item_blaster)	          return 0; // no item index.
